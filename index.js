...
function generateRandomPassword(length) {
  ...
  // Визначаємо випадковий індекс символу зі списку characters та округляємо до найбільшого цілого, яке менше.
    ...
    // console.log(index);
    resultPassword += characters.slice(index, index + 1);
...
function calculateCircleArea(radius) {
  ...
  if (typeof radius === 'number') {
    const circleSquare = Math.pow(radius, 2) * Math.PI;
...
function findMinMax(numbers) {
  ...
  if (Array.isArray(numbers)) {
    numbers.forEach(item => {
      ...
      item <= result.min ? result.min = item : result.min;
      // Якщо переданий параметр не є масивом, виводимо в консоль повідомлення про помилку.
      item >= result.max ? result.max = item : result.min;
...
function calculateHypotenuse(a, b) {
  ...
  if (typeof a !== 'number' || typeof b !== 'number') {
    ...
    console.log('Error: at least one of argument is not a number');
    // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
    return null;
  }
...
function roundObjectValues(obj) {
  ...
  if (typeof obj === 'object' && obj !== null) {
    ...
    let resultArr = arr.map(([key, value]) => {
...
function sumPositiveNumbers(numbers) {
  ...
  let result = numbers.reduce((accum, item) => {
    if (Math.sign(item) > 0) {
      return (accum += item);
    } else {
      return accum;
...
function getFractionalPart(num) {
  ...
  let newResult = Math.fround(result);
...
function compareAndRound(num1, num2) {
  ...
  const result = Math.round(Math.max(num1, num2))
...
function estimateInvestment(principal, interestRate, years) {
  ...
  if (typeof principal !== 'number' || typeof interestRate !== 'number' || typeof years !== 'number') {
    console.log('At least one of argument is not a number');
    // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
    return null;
  }
  ...
  let result = principal * Math.pow((1+interestRate), years);
  // console.log(result);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (typeof maxPrice !== 'number') {
    ...
    console.log('maxPrice is not a Number');
    // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
    return null
  }
